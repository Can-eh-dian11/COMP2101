...
#Data for hostname
hostnameOutputText=$(hostname)

#Data for LAN address
lanAddressOutputText=$(ip a s $(ip a |awk '/: e/{gsub(/:/,"");print $2}')|awk '/inet /{gsub(/\/.*/,"");print $2}')

#Data for LAN hostname
lanHostnameOutputText=$(getent hosts $(ip a s $(ip a |awk '/: e/{gsub(/:/,"");print $2}')|awk '/inet /{gsub(/\/.*/,"");print $2}')| awk '{print $2}')

#Data for external IP address
externalIpOutputText=$(curl -s icanhazip.com)

#Data for external name
externalNameData=$(getent hosts $(curl -s icanhazip.com))
externalNameOutputText=$(echo $externalNameData | awk '{print $2}')

#Data for router IP address
defaultRouterAddress=$(ip r s default| cut -d ' ' -f 3)

#Data for default router name
defaultRouterName=$(getent hosts $defaultRouterAddress | awk '{print $2}')

cat <<EOF
Hostname        : $hostnameOutputText
LAN Address     : $lanAddressOutputText
LAN Hostname    : $lanHostnameOutputText
Router Address  : $defaultRouterAddress
Router Name     : $defaultRouterName
External IP     : $externalIpOutputText
External Name   : $externalNameOutputText
EOF
